// Public browser demo: all analysis runs locally in a Web Worker.
import { request, frameSource, csvURL } from './runtime.mjs';
...
function sourceChanged() { $('run-button').disabled = state.uploading; }
...
function drawFrame() {
  ...
  image.src = frameSource(frame.index);
}
...
function renderJob(job) {
  ...
  if (complete) { $('download').href = csvURL(job.id); $('download').download = 'visionweave-detections.csv'; }
...
function renderHistory() {
  ...
  if (!state.jobs.length) {
    ...
    message.className = 'muted small'; message.textContent = 'Run the demo to start this session.';
...
function poll(id, selection) {
  state.timer = setTimeout(async () => {
    try {
      ...
      if (selection !== state.selection) return;
      report(`Could not read this run: ${error.message} Retrying…`);
      poll(id, selection);
    }
  }, 600);
}
...
async function startRun(forceDemo = false) {
  ...
  $('upload-progress').hidden = true; $('upload-progress').value = 0;
  try {
...
async function initialize() {
  ...
  } catch (error) {report(`Could not start the browser demo. ${error.message}`);}
}
initialize();
...
if (document.modelContext?.registerTool) {
  ...
  register({name: 'start_demo_analysis', title: 'Start synthetic video analysis',
    description: 'Create a new browser analysis job using synthetic frames and select it in the workspace.',
  ...
  register({name: 'get_selected_analysis_status', title: 'Read selected analysis status',
    description: 'Read current browser analysis status and result counts for the selected run.',
